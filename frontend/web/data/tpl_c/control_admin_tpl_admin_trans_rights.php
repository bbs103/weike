<?php keke_tpl_class::checkrefresh('control/admin/tpl/admin_trans_rights', '1418096911' );?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
<th><?php echo $_lang['operate'];?></th>
     ...
     <?php if(is_array($report_list)) { foreach($report_list as $v) { ?>
       <tr class="item">
         <td class="td25"><input type="checkbox" name="ckb[]" class="checkbox" value="<?php echo $v['report_id'];?>"><?php echo $v['report_id'];?></td>
         <td width="td28">
        	<?php if($v['obj']=='task') { ?><!--任务-->
<a href="<?php echo $_K['siteurl'];?>/index.php?do=task&task_id=<?php echo $v['origin_id'];?>" target="_blank">
<?php } elseif($v['obj']=='work') { ?><!--稿件-->
<a href="<?php echo $_K['siteurl'];?>/index.php?do=task&task_id=<?php echo $v['origin_id'];?>&view=work" target="_blank">
<?php } elseif($v['obj']=='order') { ?><!--商品订单-->
<a href="<?php echo $_K['siteurl'];?>/index.php?do=service&sid=<?php echo $v['obj_id'];?>" target="_blank">
<?php } elseif($v['obj']=='product') { ?><!--商品-->
<a href="<?php echo $_K['siteurl'];?>/index.php?do=service&sid=<?php echo $v['origin_id'];?>" target="_blank">
<?php } elseif($v['obj']=='seller') { ?><!--商家-->
<a href="<?php echo $_K['siteurl'];?>/index.php?do=space&member_id=<?php echo $v['to_uid'];?>" target="_blank">
<?php } ?>
        	<?php echo $_lang['view_belongs'];?><?php echo $trans_object[$v['obj']];?></a>
 </td>
 <td><?php echo kekezu::cutstr($v['report_desc'],30); ?>...</td>
         <td><a href="<?php echo $_K['siteurl'];?>/index.php?do=space&member_id=<?php echo $v['uid'];?>" target="_blank"><?php echo $v['username'];?></a></td>
 <td><a href="<?php echo $_K['siteurl'];?>/index.php?do=space&member_id=<?php echo $v['to_uid'];?>" target="_blank"><?php echo $v['to_username'];?></a></td>
         <td>
         	<?php $filename=substr($v['report_file'],strrpos($v['report_file'],'/')+1); ?>
<?php if($v['report_file']) { ?><a href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&ac=download&filename=<?php echo $filename?>&filepath=<?php echo $v['report_file'];?>"><?php echo $filename;?></a><?php } else { ?><?php echo $_lang['not_submit_attachment'];?><?php } ?>
</td>
<td><?php if($v['on_time']){echo date('Y-m-d',$v['on_time']); } ?></td>
<td><?php echo $trans_status[$v['report_status']];?></td>
<td><?php if($v['op_uid']) { ?><a href="<?php echo $_K['siteurl'];?>/index.php?do=space&member_id=<?php echo $v['op_uid'];?>" target="_blank"><?php echo $v['op_username'];?></a><?php } else { ?>--<?php } ?></td>
<td>
